...
\texttt{Attribute} \texttt{EqualityAttribute<T,U>} that indicates that we
...
will be removed and the method metadata rewriten to include the new type 
equality tags. Any use of \texttt{Cast} would rewrite it's containg 
...
The main benefit of writing in C\# should be clear, it's much easier. However we also get another less obvious benefit, that we are able to test both imperative (C\#) and functional (F\#) code. This should mean coverage over most aspects of the CLI.
