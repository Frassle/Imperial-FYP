...
\end{lstlisting}
\begin{lstlisting}[caption={LoadPpm CIL},language=cil]
...
CLI needs to load types, layout objects, resolve methods, translating CIL,
enforcing security and set up runtime context boundaries. 
...
The type system of the CLI is covered in more detail later, but briefly covered
here as well.
...
enumerations and structures. Reference types are the built in types (Object and
String), objects, interfaces, arrays, delegates, boxed value types, managed and
unmanaged pointers.
...
describe the location of that value. Some types do not fully describe any
values (abstract classes and interfaces). While every value has a type it is
not possible to infer the type given the value if the value is of a value type,
if the value is a reference type it is always possible to infer the type.

\subsubsection{The virtual execution system}

The VES\footnote{Virtual Execution System} provides an environment for executing
managed code. It supports some built-in data types, control flow constructs,
...
Managed memory supports garbage collection so does not need to be manually
deallocated. Both managed pointers and references can be changed during a
garbage collection cycle as objects are moved in memory.

Exceptions objects are any valid class type (a boxed value type or any class),
but not native integers. When an exception is throw the CLI searches for the
...
writing and rewriting via objects in the \texttt{System.Reflection}
namespace. Our implementation will extend these objects to support our new
metadata constructs. With this we can mark up standard C\# or F\# code with
special methods and attributes and rewrite the resulting assemblies to include
our metadata.
...
runtime and casts \texttt{obj} to type \texttt{U} if they are. Also an
attribute \texttt{EqualityAttribute<T,U>} that indicates that we
consider T and U equal in this context. 

The rewriter would search an assembly for uses of \texttt{Cast} and 
\texttt{EqualityAttribute<T,U>}. Any use of \texttt{EqualityAttribute<T,U>} 
will be removed and the method metadata rewritten to include the new type 
equality tags. Any use of \texttt{Cast} would rewrite it's containing 
method as if it had \texttt{EqualityAttribute<T,U>} attribute and also remove 
the call to \texttt{Cast}.
...
This system of testing has a number of advantages. Firstly it allows us to run our tests in a standard CLI runtime, secondly it means we can 
write our tests in a high level language such as C\# instead of low level CIL.

The main benefit of writing in a high level language should be clear, it's much easier. However we also get another less obvious benefit, that we are able to test both imperative (C\#) and functional (F\#) code. This should mean coverage over most aspects of the CLI.

Being able to run on a standard CLI runtime allows some verification. If a program $P$ runs correctly (doesn't throw an exception) in a runtime 
$r$ we write $\vdash_r P$. If it does not run correctly we write $\nvdash_o P$. If a program is correct in regards to type equality constraints 
then $P \diamond_tc$ is true. A program $P$ can be rewritten to $P'$ by our rewriter, this is written $P \underset{R}{\rightarrow} P'$. Lastly 
the two systems we consider are the standard CLI $o$ and our new CLI with type equality constraints $n$.

With this notation we can write the following:\\
A program that runs in the old system will continue to run in the new system.
\[ P \vdash_o \implies P \vdash_n \]
A program with correct type constraints will run correctly in the old system.
\[ P \diamond_tc \implies P \vdash_o \]
A program with correct type constraints when rewritten will run in the new system.
\[ P \diamond_tc \wedge P \underset{R}{\rightarrow} P' \implies P \vdash_n \]
A program with incorrect type constraints when rewritten will not run in the new system.
\[ ¬P \diamond_tc \wedge P \underset{R}{\rightarrow} P' \implies P \nvdash_n \]

All this together means that we can do some verification of our new system against the old system. If a program ran correctly in the old system 
it should also run correctly in the new system. If a program has correct equality type constraints then it should run in the old system (and by 
the first rule also run in the new system) and when rewritten it should run in the new system.
