...
be using these ideas but for the CLI not C\#. 
...
describes this extension. This would allow a generic type to be declared equal 
to another type, this would be checked statically at compile time. For example
...
\cite{gadts} on formalization and it's simple mapping to the CLI described here
...
The specification is 548 pages long, and while not all of it is relevent to
equality constraints it is still a large system to understand. As equality
constraints share some similarites to generic constraints we begin with an
overview of them. 
...
These rules are pretty much the same as the rules presented in Section 5 of
\cite{gadts}. 
...
Any method calls must be checked that any equality constraints on the method
...
This is an addition to the verifiability rules for the call, calli and callvirt
instructions.
...
of C code is 414,181). Mono including all it's supporting tools and libraries
...
a similar system to above. We can continue to write tests as described above,
however instead of using an assembly rewriter to transform them into an
extended CIL format we simply perform the checks that the extended verifier
