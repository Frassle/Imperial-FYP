...
Verification is done in verify.c. Additions need to be made to verify that type relationship constraints
are met when calling methods and to extend assignment compatibility to use those type relationship constraints.
...
It also requires additions to \texttt{verify_stack_type_compatibility_full} to check the new assignment 
compatibility rules.

\subsection{corlib}

The core library needs to be changed to support type equality. Specifically the reflection library defined in
mcs/class/corlib/System.Reflection\[.Emit\]. This API is predominately implement via internal calls that are 
then defined in mono/metadata/icall-def.h.

\subsection{Cecil and IKVM}

Mono.Cecil and IKVM are well known libraries for working with CIL code. They are generally considered better
than the core library System.Reflection and System.Reflection.Emit. Any extensions of the system would be well
served by extending these libraries to understand the extension as well.
